...
module Fold =
    ...
    let sum<'a when 'a :> INumber<'a>> : Fold<'a, 'a>  = create 'a.Zero (+) id
    
    /// Computes the products of all elements
    let product<'a when 'a :> INumber<'a>> = create 'a.MultiplicativeIdentity (*) id
    
    /// Computes the length of the sequence
    let length<'a> = create 0 (fun x (_ : 'a) -> x + 1) id
    
    let average<'a when 'a :> INumber<'a>> : Fold<'a, 'a> =
        create
            ('a.Zero, 'a.Zero)
    ...
    let inline sqrt a = Fold.map FSharp.Core.Operators.sqrt a
    
    /// Compute a numerically stable arithmetic mean of all elements
    let mean : Fold<double, double> =
        let step (x, n) y =
            let n' = n + 1.0
            (x + (y - x) / n'), n'
            
        create (0.0, 0.0) step fst
        
    /// Compute a numerically stable (population) variance over all elements
    let variance : Fold<double, double> =
        let step (n, m, m2) x =
            let n' = n + 1.0
            let m' = (n * m + x) / (n + 1.0)
            let delta = x - m
            let m2' = m2 + delta * delta * n / (n + 1.0)
            (n', m', m2')
        create (0.0, 0.0, 0.0) step (fun (n, _, m2) -> m2 / n)
